...
def admin_dashboard():
    ...
    return render_template('dashboard_admin.html', user=user, members=members, books=books,  
                           member_count=member_count, book_count=book_count,
                           fines=FineHistory.query.all())
...
def user_dashboard():
    ...
    book_count = len(books)
    return render_template('dashboard_user.html', user=user, books=books, book_count=book_count, 
                           borrowed=borrowed, fines=fines)
...
def check_borrow_reminders():
    ...
    for borrow in borrows:
        due_date = borrow.borrow_date.date() + timedelta(days=2)
